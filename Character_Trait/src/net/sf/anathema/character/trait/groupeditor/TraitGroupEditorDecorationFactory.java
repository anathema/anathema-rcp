...
public class TraitGroupEditorDecorationFactory {
	...
	public TraitGroupEditorDecorationFactory() {
		this(new EclipseExtensionProvider().getExtensions(
				...
				"net.sf.anathema.character.trait.editordecorations")); //$NON-NLS-1$
	}
	...
	public Collection<ITraitGroupEditorDecoration> create() {
		...
		for (IPluginExtension extension : extensions) {
			for (IExtensionElement element : extension.getElements()) {
				try {
					decorations.add(element.getAttributeAsObject(
							"class", ITraitGroupEditorDecoration.class)); //$NON-NLS-1$
