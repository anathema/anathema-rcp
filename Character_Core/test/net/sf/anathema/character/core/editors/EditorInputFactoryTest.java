...
public class EditorInputFactoryTest {
  ...
  public void createsWithModelDisplayNameProvider() throws Exception {
    ...
    EasyMock.expect(folder.getName()).andReturn("Character").anyTimes();
    IModelDisplayConfiguration displayConfiguration = EasyMock.createNiceMock(IModelDisplayConfiguration.class);
    IDisplayNameProvider provider = new ModelDisplayNameProvider("Model", new CharacterDisplayNameProvider(folder));
    EasyMock.expect(displayConfiguration.getDisplayName()).andReturn("Model");
