...
public class PlotUnitProvider {
	...
	public PlotUnitProvider() {
		...
		for (IPluginExtension extension : new EclipseExtensionProvider()
				.getExtensions(PlotPlugin.ID, UNITS_EXTENSION_POINT)) {
			for (IExtensionElement element : extension.getElements()) {
				ExtensionPlotUnit unit = new ExtensionPlotUnit(extension
						.getContributorId(), element);
				String parentId = element.getAttribute("parentId"); //$NON-NLS-1$
				if (parentId == null) {
					Ensure.ensureArgumentNull(
							"Only one root unit may be defined.", root); //$NON-NLS-1$
					this.root = unit;
				} else {
