...
import net.sf.anathema.map.view.gisterm.NullClosableValidator;
...
public class LayerPanelPopupFactory {
  ...
  public JPopupMenu createPopupMenu(final IMapSelectionContext context) {
    final IThemeSelectionModel selectionModel = mapModel.getSelectionModel();
    final ITheme[] selectedThemes = selectionModel.getSelectedThemes();
    Ensure.ensureArgumentTrue("At least one theme must be selected.", selectedThemes.length > 0); //$NON-NLS-1$
  ...
  private JPopupMenu createGroupLayerPopup(final GroupTheme groupTheme, final IMapSelectionContext context) {
    ...
    popupMenu.add(new RemoveLayersAction(groupTheme, mapModel, new NullClosableValidator()));
    final UnGroupLayersAction unGroupLayersAction = new UnGroupLayersAction(groupTheme, mapModel);
    popupMenu.add(unGroupLayersAction);
    unGroupLayersAction.setEnabled(customizations.isThemeGroupingActionsEnabled());
    ...
    addAttributeSearchActionIfRequired(popupMenu, context, new ITheme[] { groupTheme });
    popupMenu.addSeparator();
    popupMenu.add(new ThemePropertiesAction(mapModel, groupTheme, new NullClosableValidator()));
    return popupMenu;
  }

  private void addLabellingMenu(final JPopupMenu popupMenu, final GroupTheme groupTheme) {
    ...
    final JMenu labelMenu = createLabelMenu(featureLayers.toArray(new FeatureLayer[featureLayers.size()]));
    popupMenu.add(labelMenu);
  }

  private JMenu createLabelMenu(final IFeatureLayer[] layers) {
    return new FeatureLayerLabelMenuFactory(new IFeatureLayerLabelConfiguration() {
      @Override
      public void setLabelMode(final FeatureLabelMode itemMode) {
        for (final IFeatureLayer featureLayer : layers) {
          ((FeatureLayer) featureLayer).setFeatureLabelMode(itemMode);
        }
      }

      @Override
      public FeatureLabelMode getLabelMode() {
        return ((FeatureLayer) layers[0]).getLabelConfiguration().getLabelMode();
      }
    }).createLabelMenu();
  }

  private void addGroupExtensionActions(final JPopupMenu popupMenu, final IGroupThemeAnnex[] extensions) {
    final List<Action> actions = new ArrayList<Action>();
    for (int extensionIndex = 0; extensionIndex < extensions.length; extensionIndex++) {
      final IGroupThemeAnnexActionFactory factory = GroupThemeActionRegistryFactory.getInstance();
      Collections.addAll(actions, factory.getActions(extensions[extensionIndex]));
    }
    if (actions.size() > 0) {
      popupMenu.addSeparator();
    }
  ...
  private void addAction(final JPopupMenu popupMenu, final Action action) {
    if (action instanceof SmartToggleAction) {
      popupMenu.add(ActionWidgetFactory.createToggleMenuItem((SmartToggleAction) action));
    }
    else {
  ...
  private void addShowErrorActionIfNecessary(final JPopupMenu popupMenu, final ITheme selectedTheme) {
    final Action action = ShowThemeErrorAction.createShowThemeErrorActionIfNecessary(selectedTheme);
    if (action != null) {
      popupMenu.add(action);
      popupMenu.addSeparator();
    }
  ...
  private void addZoomToScaleActionIfNecessary(
      final JPopupMenu popupMenu,
      final IMapSelectionContext context,
      final ITheme selectedTheme) {
    final IScaleRange[] scaleRanges = ThemeScaleUtilities.getScaleRanges(selectedTheme);
    if (context.getDisplayContext().getDisplay() != null
        && !ScaleRangeUtilities.contains(mapModel.getViewProperties().getScale(), scaleRanges)) {
      popupMenu.add(new ZoomToScaleRangeAction(context.getDisplayContext().getDisplay(), scaleRanges));
    }
  }

  private JPopupMenu createMultipleLayerPopup(final ITheme[] themes, final IMapSelectionContext context) {
    final JPopupMenu popupMenu = new JPopupMenu();
    popupMenu.add(new ZoomToThemeExtentAction(mapModel, themes));
    IThemeClosableValidator validator;
    popupMenu.add(new RemoveLayersAction(themes, mapModel, new NullClosableValidator()));
    final GroupLayersAction groupLayersAction = new GroupLayersAction(themes, mapModel);
    groupLayersAction.setEnabled(customizations.isThemeGroupingActionsEnabled());
  ...
  private void addMulitpleFeatureLayerLabelMenu(final JPopupMenu popupMenu, final ITheme[] themes) {
    if (!containsFeatureLayer(themes)) {
      return;
    }
    final JMenu labelMenu = createLabelMenu(FeatureLayerUtilities.getAllFeatureLayers(themes));
    popupMenu.add(labelMenu);
  }

  private void addAttributeSearchActionIfRequired(
    ...
    builder.addDefaultSelectionModelActions(FeatureLayerUtilities.getAllSelectionModels(themes));
  ...
  private boolean containsFeatureLayer(final ITheme[] themes) {
    final IFeatureLayer[] allFeatureLayers = FeatureLayerUtilities.getAllFeatureLayers(themes);
    return allFeatureLayers.length > 0;
  }

  private JPopupMenu createLayerPopup(final LayerTheme layerTheme, final IMapSelectionContext context) {
    final GenericLegendLayer layer = (GenericLegendLayer) layerTheme.getLayer();
    ...
    popup.add(new RemoveLayersAction(layerTheme, mapModel, new NullClosableValidator()));
    addSaveLayerActions(popup, layerTheme);
    final ThemeLayerId themeLayerId = new ThemeLayerId(layerTheme.getId(), layer.getId());
    addMenusFromFactories(popup, context);
    addSpecializedLayerMenu(popup, layer, themeLayerId, context);
    return popup;
  }

  private void addMenusFromFactories(final JPopupMenu popup, final IMapSelectionContext context) {
    final IMapMenuItemFactory[] menuItemFactories = menuFactoryRegistry.getLegendContextMenuItemFactories();
  ...
  private void addSpecializedLayerMenu(
      final JPopupMenu popup,
      final GenericLegendLayer layer,
      final ThemeLayerId themeLayerId,
      final IMapSelectionContext context) {
    final SpecializedLayerMenuFactory menuFactory = new SpecializedLayerMenuFactory(specializedMenuFactoryRegistry);
    final JMenuItem[] specializedItems = menuFactory.addSpecialActions(layer, themeLayerId, context);
