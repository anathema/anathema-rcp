...
public class RegisteredContentEncoderProvider implements
		IContentEncoderProvider {
	...
	public RegisteredContentEncoderProvider() {
		this(
				new Logger("net.sf.anathema.character.sheet"), new EclipseExtensionProvider().getExtensions(CharacterSheetPluginConstants.PLUGIN_ID, ENCODERS_EXTENSION_POINT)); //$NON-NLS-1$
	}

	public RegisteredContentEncoderProvider(ILogger logger,
	...
	@Override
	public IPdfContentBoxEncoder getContentEncoder(String encoderName,
			IModelContainer character) {
		for (IPluginExtension extension : extensions) {
			for (IExtensionElement element : extension.getElements()) {
				String fieldId = element.getAttribute(ATTRIB_FIELD_ID);
				if (fieldId.equals(encoderName)) {
					try {
						return element.getAttributeAsObject(ATTRIB_CLASS,
	...
	@Override
	public IDynamicPdfContentBoxEncoder getDynamicContentEncoder(
			String encoderName, IModelContainer character) {
		IPdfContentBoxEncoder contentEncoder = getContentEncoder(encoderName,
				character);
