...
public class PointConfigurationExtensionPoint implements
		IPointConfigurationProvider {
	...
	private static final ILogger logger = new Logger(
			PointPluginConstants.PLUGIN_ID);

	public IPointConfiguration[] getExperiencePointConfigurations(
			ICharacterTemplateProvider provider, ICharacterId characterId) {
		return getPointConfigurations(provider, characterId,
				TAG_EXPERIENCE_POINT_CALCULATOR);
	}

	public IPointConfiguration[] getBonusPointConfigurations(
			ICharacterTemplateProvider provider, ICharacterId characterId) {
		return getPointConfigurations(provider, characterId,
				TAG_BONUS_POINT_CALCULATOR);
	}
	...
	private IPointConfiguration[] getPointConfigurations(
		...
		for (IPluginExtension extension : getPluginExtensions()) {
			for (IExtensionElement configurationElement : extension
					.getElements()) {
				if (pointCalculatorElementName.equals(configurationElement
						.getName())) {
					String groupReference = configurationElement
							.getAttribute(ATTRIB_GROUP_REFERENCE);
					IPointHandler handler = getPointHandler(configurationElement);
					if (handler != null) {
						handlersByGroupId.add(groupReference, handler);
					}
				} else if (TAG_POINT_GROUP.equals(configurationElement
						.getName())) {
					String groupId = configurationElement
							.getAttribute(ATTRIB_GROUP_ID);
					String name = configurationElement
							.getAttribute(ATTRIB_NAME);
					String modelId = configurationElement
							.getAttribute(ATTRIB_MODEL_ID);
					if (template.supportsModel(modelId)) {
						configurationsByGroupId.put(groupId,
								new PointConfiguration(name));
					}
				}
			}
		}
		...
		for (String groupId : configurationsByGroupId.keySet()) {
			...
			PointConfiguration configuration = configurationsByGroupId
					.get(groupId);
			configuration.addHandlers(handlers);
			configurations.add(configuration);
		}
		return configurations.toArray(new IPointConfiguration[configurations
				.size()]);
	}

	private IPointHandler getPointHandler(IExtensionElement element) {
		try {
			return element.getAttributeAsObject(ATTRIB_HANDLER_CLASS,
					IPointHandler.class);
	...
	private IPluginExtension[] getPluginExtensions() {
		return new EclipseExtensionProvider().getExtensions(
				PointPluginConstants.PLUGIN_ID, CONFIGURATIONS_EXTENSION_POINT);
	}
}