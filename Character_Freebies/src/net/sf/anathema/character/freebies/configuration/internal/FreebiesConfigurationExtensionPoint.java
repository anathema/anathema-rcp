...
public class FreebiesConfigurationExtensionPoint implements
		IFreebiesConfigurationsProvider {
	...
	public IFreebiesConfiguration[] get(ICharacterId characterId) {
		String templateId = new CharacterTemplateProvider().getTemplate(
				characterId).getId();
		...
		for (IPluginExtension extension : extensionProvider.getExtensions(
			...
			for (IExtensionElement element : extension.getElements()) {
				...
				try {
					IFreebiesHandler handler = element.getAttributeAsObject(
							"handlerClass", IFreebiesHandler.class); //$NON-NLS-1$
					if (creditManager.hasCredit(templateId, handler
							.getCreditId())) {
						configurations.add(new FreebiesConfiguration(entryName,
								handler, creditManager));
					}
				} catch (ExtensionException e) {
					configurations.add(new MissingFreebiesConfiguration(
							entryName));
				}
			}
		}
		return configurations.toArray(new IFreebiesConfiguration[configurations
				.size()]);
	}
}