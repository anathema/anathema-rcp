...
public class CreditManagerTest {
	...
	public CreditManagerTest(String creditId, boolean expectedAvailability,
			int expectedCredit) {
	...
	public void createManager() throws Exception {
		this.characterId = TemplateProviderObjectMother
				.createCharacterId(TEMPLATE_ID);
		IExtensionProvider provider = EasyMock
				.createMock(IExtensionProvider.class);
		...
		this.manager = new CreditManager(provider, TemplateProviderObjectMother
				.createTemplateProvider(characterId, TEMPLATE_ID));
	}

	@Test
	public void assertAvailability() {
		assertEquals(expectedAvailability, manager.hasCredit(TEMPLATE_ID,
				CREDIT_ID));
	}
	...
	private void createExtension(FakeExtensionElement element,
			IExtensionProvider provider) {
		IExtensionElement[] elements = new IExtensionElement[] { element };
		IPluginExtension extension = EasyMock
				.createMock(IPluginExtension.class);
		...
		EasyMock.expect(
				provider.getExtensions(
						...
						CREDITS_EXTENSION_POINT)).andReturn(extensions)
				.anyTimes();
