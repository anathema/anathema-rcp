...
public class HasUnspentFreebies_Test {
  ...
  @Test
  public void hasNoUnspentPointIfSpentFreebiesEqualCredit() throws Exception {
    expect(freebiesHandler.getPoints(characterId, CREDIT)).andReturn(CREDIT);
    replay(freebiesHandler);
    assertThat(hasUnspentFreebies.evaluate(identifier), is(false));
  }

  @Test
  public void hasUnspentPointIfSpentFreebiesAreBelowCredit() throws Exception {
    expect(freebiesHandler.getPoints(characterId, CREDIT)).andReturn(CREDIT - 1);
    replay(freebiesHandler);
    assertThat(hasUnspentFreebies.evaluate(identifier), is(true));
  }
}