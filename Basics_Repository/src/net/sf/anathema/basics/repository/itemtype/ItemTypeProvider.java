...
public class ItemTypeProvider {
	...
	public Iterable<IItemType> getItemTypes() {
		...
		for (IPluginExtension pluginExtension : configurationProvider
				.getExtensions(RepositoryPlugin.ID, ITEMTYPES_EXTENSION_POINT)) {
			for (IExtensionElement extensionElement : pluginExtension
					.getElements()) {
				itemTypes.add(new ItemType(pluginExtension.getContributorId(),
						extensionElement));
			}
		}
		return itemTypes;
