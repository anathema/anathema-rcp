...
public class RepositoryView extends ViewPart implements IResourceSelector,
		ILinker, ICollapsableTree, IExpandableTree {
	...
	public void createPartControl(Composite parent) {
		this.viewLinker = new ViewEditorLinker(getSite().getWorkbenchWindow(),
				this);
		disposables.add(viewLinker);
		this.viewer = new TreeViewer(parent, SWT.MULTI | SWT.H_SCROLL
				| SWT.V_SCROLL);
		this.editorLinker = new EditorViewLinker(
				getSite().getWorkbenchWindow(), viewer);
		...
		viewer.addOpenListener(new IOpenListener() {
			public void open(OpenEvent event) {
				StructuredSelection selection = (StructuredSelection) event
						.getSelection();
				IViewElement element = (IViewElement) selection
						.getFirstElement();
				try {
					element.openEditor(getViewSite().getPage());
				} catch (PartInitException e) {
					RepositoryPlugin.getDefaultInstance().log(IStatus.ERROR,
							Messages.RepositoryView_OpenEditorErrorMessage, e);
				}
			}
		});
		...
		final TreeViewRefresher treeViewRefresher = new TreeViewRefresher(
				viewer, parent.getDisplay());
	...
	private void initDragAndDrop() {
		IPluginExtension[] extensions = new EclipseExtensionProvider()
				.getExtensions(RepositoryPlugin.ID,
						DRAG_AND_DROP_EXTENSION_POINT);
		for (IPluginExtension extension : extensions) {
			IExtensionElement[] elements = extension.getElements();
			for (IExtensionElement extensionElement : elements) {
				try {
					IRepositoryDND dnd = extensionElement.getAttributeAsObject(
							"class", IRepositoryDND.class); //$NON-NLS-1$
					dnd.initDragAndDrop(viewer, getViewSite());
				} catch (ExtensionException e) {
					RepositoryPlugin.getDefaultInstance().log(IStatus.ERROR,
							Messages.RepositoryView_InitializeDndErrorMessage,
							e);
				}
			}
		}
	...
	public void setSelection(IResource resource) {
		new RepositoryResourceSelector(contentProvider, new RevealingSelector(
